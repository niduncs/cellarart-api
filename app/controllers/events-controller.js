...
EventsController.prototype.findEventById = function (req, res) {
    this.db('events').where({ id: req.params.event_id }).select().then((value) => { 
        if (value[0]) {
            res.status(200);
...
EventsController.prototype.findAllEvents = function (req, res) {
    this.db('events').select().then((value) => {
        if (value.length > 0) {
            res.status(200);
...
EventsController.prototype.addEvent = function (req, res) {
    this.db('events').insert(req.body).returning('*').then((value) => {
        if (value[0]) {
            res.status(200);
            return res.json(value[0]);
        } else {
...
EventsController.prototype.editEvent = function (req, res) {
    this.db('events').where({ id: req.params.event_id }).update(req.body).returning('*').then((value) => {
        if (value[0]) {
            res.status(200);
            return res.json(value[0]);
        } else {
...
EventsController.prototype.deleteEvent = function (req, res) {
    this.db('events').where({ id: req.params.event_id }).del().then((v) => {
        res.status(200);
        return res.send(v);
    }).catch((e) => {
        console.error(e);
